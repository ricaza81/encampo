...
class SupportAdminController extends Controller {
	...
	*/
	...
	public function index()
	{
		...
		$tickets=Tickets::Where('status', '=', '0')->limit(15)->get();
	//	$kb_count=Knowledgebase::count();
		...
		if ($usuarioactual->tickets==1) {

		return view('support.agent/home')->with('tickets', $tickets)
 ...
 $id=\Auth::user()->id;
    ...
    $tickets=Tickets::where('user_id','=', $id)/*->where('status', '=', $status)*/->paginate('25');
    return view('support.agent/tickets')->with("usuario", $usuarioactual)->with('tickets', $tickets);/*->with('status', $param);*/
  }
...
/*	public function tickets($param)
	...
	*/
	...
	$usuarioactual=\Auth::user();//$products=Products::get();
	//$members=Members::get();
...
->with("usuario", $usuarioactual);
  ...
  /*      $mem=User::find($id);
   ...
   mail($to,$subject,$data['msg'],$headers);*/
     ...
     return redirect('customer/ticket/view/'.$kb['id']);
   }
...
$events_day=count($events);
    ...
    return view('support.agent/main')->with('ticket', $ticket)->with("usuario", $usuarioactual)->with("events_day",$events_day)->with('tickets', $tickets)->with('msgs', $msgs)->with('status', $status);
  }
 ...
 $id=\Auth::user()->id;
  ...
  $tickets->status=0;
  $tickets->save();
    return redirect('agent/ticket/view/'.$formData['ticket']);
    }
...
$events_day=count($events);
		$usuarioactual=\Auth::user();
	//	$tickets=Tickets::all();
		$tickets=Tickets::groupby('user_id')->distinct()->get();
			return view('support.agent/members')->with("events_day",$events_day)->with("usuario", $usuarioactual)->with('tickets', $tickets);
	}
...
$events_day=count($events);
		...
		return view('support.agent/settings')->with("events_day",$events_day)->with("usuario", $usuarioactual)->with('settings', $settings);
	}
    ...
    foreach ($formData as $data=>$value) {
      $affected = DB::table('tbl_settings')->where('setting', '=', $data)->update(array("value" => $value));
       ...
       return redirect('agent/settings')->with('message', 'Updated Successfully');
